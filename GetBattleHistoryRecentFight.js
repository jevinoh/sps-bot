...
async function getPlayerating(player = '') {
    ...
    if(rating >= 500 && rating <= 1100)
    {
        userCounter++;
        console.error('userCounter [' + userCounter +']');
        return 1;
    }
    else
    {
        console.error('Not a bronze rank user, rating[' + rating + ']');
        return 0;
    }
}
...
async function getBronzeLeaderBoard() {
    ...
    Promise.all(battles).then(() => {
        if(userCounter >= 150)
        {
            ...
            var strFile = 'data/bronzeRank/history' + '_' + counter + '.json';

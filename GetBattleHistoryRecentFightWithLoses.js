...
async function getPlayerating(player = '') {
    ...
    .catch((error) => {
        const secondaryPlayerInfo = fetch('https://api.splinterlands.io/players/details?name=' + player)
        .then((response) => {
            if (!response.ok) {
    ...
    if(rating >= 500 && rating <= 1200 && league <= 3)
    {
        userCounter++;
...
async function getBronzeLeaderBoard() {
        ...
        getBattleHistory(user)
            .then(battles => battles.map(
                battle => {
                    ...
                    if (details.type != 'Surrender' && battle.winner != 'DRAW') {
                        let loser;
                        let winner;
                        ...
                        if (battle.winner && battle.winner == battle.player_1) {
                            ...
                            // console.log("monstersDetails :" +  JSON.stringify(monstersDetails));
                        ...
                        // console.log("Loser  :" +  JSON.stringify(loser, null, 2));
    ...
    Promise.all(battles).then(() => {
        if(userCounter >= 200)
        {
            ...
            var strFile = 'data/bronzeRankWithAllStatus/history' + '_' + counter + '.json';
...
(async () => {
    ...
    // const cleanBattleList = battlesListWave.filter(x => x != undefined && x.summoner_id == 16)
    // var strFile = 'data/battleHistoryWavesmith/history' + '_' + 4 + '.json';
    // console.log("json file [" + strFile + "]");
    // fs.writeFile(strFile, JSON.stringify(cleanBattleList, null, 2), function (err) {
    //     if (err) {
    //         console.log(err);
